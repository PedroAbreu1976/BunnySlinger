...
internal class BunnyMqRegister : IBunnyRegister
{
	...
	private readonly List<Func<Task>> _messages = [];
    ...
    public void AddBunnyCatcher(Type handlerType, Type msgType)
    {
        _handlers.Add(async () => {
			...
			consumer.ReceivedAsync += async (model, ea) =>
			{
				...
				var message = JsonSerializer.Deserialize(json, msgType) as IBunny;
				if (message != null)
				{
					bool result;
    ...
    public void AddBunnyCatcher<T>(IBunnyCatcher<T> handler)
		where T : IBunny
	{
		_handlers.Add(async () => {
			var channel = await _channelProvider.Create();
    ...
    public async Task RegisterAsync() {
		...
		try
		{
			foreach (var action  in _messages) {
				await action();
            }
